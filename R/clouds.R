                       ...
                       lwd = 1, max = 2500, trace = TRUE, opacity = 0.25)
{
  ...
  if (is.null(which)) which <- 1:g
    ...
    for(a in 1:length(control.output$which))
    {
      ...
      if(ncol(bp$Z)==2){calc <- calc.alpha.bags(Zgroup, aa=control.output$alpha[a], approx.limit=control.output$max)$xy[,1:2]
      } else if(ncol(bp$Z)==1){
...
ellipses <- function(bp, df=2, kappa = NULL, which = NULL, alpha = 0.95,
                     col = bp$sample$col[which], lty = 1, lwd = 1, opacity = 0.5, trace = TRUE)
{
