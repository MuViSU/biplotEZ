...
samples <- function (bp,  which = 1:bp$g, col = ez.col, pch = 16, 
                     ...
                     connect.lty = 1,  connect.lwd = 1, opacity = 0.7)
{
  g <- bp$g
  n <- bp$n
  p <- bp$p
...
#'
#' @usage
#' means (bp,  which = NULL, col = NULL, pch = 1, cex = 1.5, label = FALSE,
...
means <- function (bp,  which = NULL, col = NULL,
                     pch = 1, cex = 1.5, label = FALSE, label.col=NULL, label.cex = 0.75,
                     label.side = "bottom", label.offset = 0.5, opacity = 0.7)
{
...
#' Defaults to zero for each axis. Only used when the dimension of the biplot is two.
#' @param calibrated.axes logical, whether calibrated axes should be displayed on the biplot
...
#' orthogx = 0, orthogy = 0,calibrated.axes = TRUE,unit.circle=FALSE)
#' @aliases axes
#'
#' @export
#'
                  ...
                  orthogx = 0, orthogy = 0,calibrated.axes = TRUE,unit.circle=FALSE)
{
  ...
  bp$axes = list(which = which, col = col, lwd = lwd, lty = lty, label.dir = label.dir, label.col = label.col, label.cex = label.cex,
                 ...
                 names = ax.names, orthogx = orthogx, orthogy = orthogy,calibrated.axes = calibrated.axes,
...
#' @noRd
#'
control.alpha.bags <- function (g, g.names, alpha, which, col, lty, lwd, max, opacity)
{
