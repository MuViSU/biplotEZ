...
#' @param original coordinates of class means - matrix [x,y]
...
TrueBoundary<-function(seg,original,points){
  ...
  for(i in 1:nrow(midpoints)){
    close_point<-min.euc.dist(midpoints[i,],original)
...
#'
#' @param x Matrix of coordinates - class centers. [x,y]
...
predict.regions <- function (x, bounds=graphics::par("usr"))
{
  ...
  if(nrow(TruePoints)==2){
    ...
    for(i in 1:4){
      idx<-min.euc.dist(bounds_coors[i,],x)[1]
