...
# Load FORTRAN code
dyn.load("C:\\Users\\29827094\\Documents\\GitHub\\biplotEZ\\src\\biplotEZ.dll")
...
#   }
  ...
  optimtouse <- function(Bvec) {
    ...
    if (length(Bvec) != 2*(u+v)) {
      if (length(Bvec) < 2*(u+v)) {
        Bvec <- c(Bvec, rep(0, 2*(u+v) - length(Bvec)))
      } else {
        Bvec <- Bvec[1:(2*(u+v))]
      }
    }
    ...
    # Check for NA/Inf
    if (any(is.na(Bvec)) || any(is.infinite(Bvec))) {
      Bvec[is.na(Bvec) | is.infinite(Bvec)] <- rnorm(sum(is.na(Bvec) | is.infinite(Bvec)), 0, 0.01)
    }
...
#   # Initial coefficients - ensure correct dimensions
#   Bvec_initial <- rep(Bvec, length.out=2*(u+v))
#   if (length(Bvec) < 2*(u+v)) {
#     Bvec_initial[(length(Bvec)+1):(2*(u+v))] <- rnorm(2*(u+v) - length(Bvec), 0, 0.01)
#   }
  
#   # Scale initial values to avoid numerical issues
#   if (max(abs(Bvec_initial)) > 5) {
#     Bvec_initial <- Bvec_initial / max(abs(Bvec_initial)) * 1
#   }
  
#   # For v > 0, try starting from smaller values
#   if (v > 0 && max(abs(Bvec_initial)) > 2) {
#     Bvec_initial <- Bvec_initial * 0.5
#   }
