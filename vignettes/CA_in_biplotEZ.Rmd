...
As explained in `biplotEZ`, the quality of the biplot is measured by the ratio of the variance explained (sum of the squared singular values of the utilised ($M$) components) and the total variance (sum of all squared singular values ($p$)).

$$
\frac{\sum_{m=1}^{M}\lambda_m^2}{\sum_{m=1}^{p}\lambda_m^2}
$$
The `adequacy` refers to the representation of the variables. In `CA()` the factor variable represented in the columns is treated as the variables and is calculated as explained in `biplotEZ`:
...
The `row.predictivities` are calculated as follows (@UB2011: Page 299):
...
The `col.predictivities` are calculated as follows (@UB2011: Page 299):
