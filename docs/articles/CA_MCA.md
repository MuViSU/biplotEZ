...
centering are not allowed on the two-way contingency table and a warning
...
![](CA_MCA_files/figure-html/unnamed-chunk-9-1.png)

### `Variant="Stand"`

To construct the CA biplot for row standard coordinates:

`ca.out`` ``<-`` `[`biplot`](../reference/biplot.md)`(``HairEyeColor``[``,,``2``]``, center ``=`` ``FALSE``)`` ``|>`` `[`CA`](../reference/CA.md)`(``variant ``=`` ``"Stand"``)`` ``|>`` `` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](CA_MCA_files/figure-html/unnamed-chunk-10-1.png)

### `Variant="Symmetric"`

To construct the symmetric CA map:

`ca.out`` ``<-`` `[`biplot`](../reference/biplot.md)`(``HairEyeColor``[``,,``2``]``, center ``=`` ``FALSE``)`` ``|>`` `` `` `[`CA`](../reference/CA.md)`(``variant ``=`` ``"Symmetric"``)`` ``|>`` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](CA_MCA_files/figure-html/unnamed-chunk-11-1.png)
...
[`biplot`](../reference/biplot.md)`(``HairEyeColor``[``,,``2``]``, center ``=`` ``FALSE``)`` ``|>`` `[`CA`](../reference/CA.md)`(``variant ``=`` ``"Symmetric"``)`` ``|>`` `` `` `[`samples`](../reference/samples.md)`(``pch ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``0``,``2``)``)`` ``|>`` `[`interpolate`](../reference/interpolate.md)`(``newdata ``=`` ``HairEyeColor``[``,,``1``]``)`` ``|>`` `` `` `[`newsamples`](../reference/newsamples.md)`(``col ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"orange"``,``"purple"``)``, pch ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``15``,``17``)``, label ``=`` ``TRUE``)`` ``|>`` `` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`` `

![](CA_MCA_files/figure-html/unnamed-chunk-13-1.png)
...
[`biplot`](../reference/biplot.md)`(``HairEyeColor``[``,,``2``]``, center ``=`` ``FALSE``)`` ``|>`` `[`CA`](../reference/CA.md)`(``variant ``=`` ``"Princ"``)`` ``|>`` `` `` `[`samples`](../reference/samples.md)`(``col ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"cyan"``,``"purple"``)``, pch ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``15``,``17``)``, `` `` label.side ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"bottom"``,``"top"``)``, label.cex ``=`` ``1``)`` ``|>`` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](CA_MCA_files/figure-html/unnamed-chunk-14-1.png)

[`biplot`](../reference/biplot.md)`(``HairEyeColor``[``,,``2``]``, center ``=`` ``FALSE``)`` ``|>`` `[`CA`](../reference/CA.md)`(``variant ``=`` ``"Symmetric"``)`` ``|>`` `` `` `[`samples`](../reference/samples.md)`(``col ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"forestgreen"``,``"magenta"``)``, pch ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``12``,``17``)``, `` `` label.side ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"top"``,``"bottom"``)``)`` ``|>`` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](CA_MCA_files/figure-html/unnamed-chunk-15-1.png)
...
[`biplot`](../reference/biplot.md)`(``SACrime``, center ``=`` ``FALSE``)`` ``|>`` `` `` `[`CA`](../reference/CA.md)`(``variant ``=`` ``"Symmetric"``, lambda.scal ``=`` ``TRUE``)`` ``|>`` `` `` `[`samples`](../reference/samples.md)`(``col ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"cyan"``,``"purple"``)``, pch ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``15``,``17``)``, `` `` label.side ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"bottom"``,``"top"``)``)``|>`` `` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](CA_MCA_files/figure-html/unnamed-chunk-17-1.png)

## What is a Multiple Correspondence Analysis?

Now we shift our attention to data sets with more than two categorical
variables. Consider a data matrix $`\mathbf{X}^{*}:n \times p`$
containing data on $`n`$ objects and $`p`$**categorical** variables.
Before approximating this data matrix to be represented in a 2D biplot,
we recode the variables as dummy variables in the form of an indicator
matrix, $`\mathbf{G}`$. Each variable $`p`$ consists of $`k`$ category
levels, therefore the indicator matrix $`\mathbf{G}`$ has $`n`$ rows and
$`\sum_{j=1}^pk_j`$ columns. This indicator matrix is treated as the
data matrix in CA, which includes the steps of obtaining a
correspondence matrix, row and column masses expressed as diagonal
matrices before obtaining the SVD of the matrix of standardised Pearson
residuals ($`\mathbf{S}`$). since it is weighted by the row and column
masses before SVD (Gower et al. (2011); Greenacre (2017)):

``` math
\begin{aligned}
\text{SVD}(\mathbf{S}) &= \text{SVD}\left(\mathbf{D_r^{-\frac{1}{2}}}(\mathbf{G}-\mathbf{rc^\prime})\mathbf{D_c^{-\frac{1}{2}}}\right)\\&= \mathbf{U\Lambda V^\prime}
\end{aligned}
```

There are other known approaches to MCA, for example, utilising a Burt
matrix in the place of the indicator matrix. Also, considering different
distance metrics, such as the extended mathcing coefficient (Gower et
al. (2011)). These are not covered in the current version of `biplotEZ`.

## MCA biplot

The MCA biplot consists of two types of coordinates and no axes. In
total there are $`n`$ samples coordinates and $`\sum_{j=1}^pk_j`$
category level point (CLP) coordinates. Typically, these coordinates are
illustrated by different plotting characters and colours to ease visual
interpretation. The distances between the coordinates are meaningful and
reveal association between objects, between responses (category levels),
as well as inter-relationships between responses and objects. These
associations are approximated in a two-dimensional display, as the
quality of the display increases, the proximity of the coordinates
reflect the true response profiles captured in the data matrix.

### Coordinates

Again, the coordinates representing the objects and variables should
have different contribution of the singular values. Typically, the
sample coordinates are presented in principal coordinates and the
category level point (CLP) coordinates are presented in standard
coordinates with some being located on the vertices of the plot (Gabriel
(1971)).

\$\$ \begin{aligned} \text{Sample principal coordinates:} \hspace{0.5
cm}&\mathbf{D_r^{-\frac{1}{2}}}\mathbf{U}\\ \text{Category level
standard coordinates:} \hspace{0.5
cm}&\mathbf{D_c^{-\frac{1}{2}}}\mathbf{V} \end{aligned} \$\$ The scaling
of the coordinates by the row and column masses is a default approach
adopted by Greenacre (2017). However, various valid alternative scaling
options are available, but beyond the scope of `biplotEZ`.

### Measures of fit

`quality` is available for MCA biplots and is typically referred to as
the total inertia in MCA literature. Again, it is measured by the ratio
of the variance explained (sum of the squared singular values of the
utilised ($`M`$) components) and the total variance (sum of all squared
singular values ($`p`$)).
...
The function [`MCA()`](../reference/MCA.md) requires a multivariate
categorical data matrix as input and will return an object of class
`MCA` and `biplot`. If a matrix with both numeric and categorical
variables are passed through the [`biplot()`](../reference/biplot.md)
function, only the categorical variables will be considered for MCA. As
the variables are categorical, scaling and centering are not suitable
transformations and a warning will be given if either `scale` or
`center` is specified as `TRUE` in biplot()\`.

The full `HairEyeColor` is considered for the MCA examples.

[`head`](https://rdrr.io/r/utils/head.html)`(`[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``HairEyeColor``)``)`` ``#> Hair Eye Sex Freq`` ``#> 1 Black Brown Male 32`` ``#> 2 Brown Brown Male 53`` ``#> 3 Red Brown Male 10`` ``#> 4 Blond Brown Male 3`` ``#> 5 Black Blue Male 11`` ``#> 6 Brown Blue Male 50`

As the final column represents the frequency of the co-occurrences of
the category elvels of the Hair, Eye and Sex variables, it is treated as
numeric and will not be passed to the [`MCA()`](../reference/MCA.md)
function.

[`biplot`](../reference/biplot.md)`(`[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``HairEyeColor``)``, center ``=`` ``FALSE``)`` ``|>`` `[`MCA`](../reference/MCA.md)`(``)`` `` ``#> Object of class biplot, based on 32 samples and 4 variables.`` ``#> 1 numeric variable.`` ``#> 3 categorical variables.`

The default arguments will result in the following biplot:

[`biplot`](../reference/biplot.md)`(`[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``HairEyeColor``)``, center ``=`` ``FALSE``)`` ``|>`` `[`MCA`](../reference/MCA.md)`(``)`` ``|>`` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](CA_MCA_files/figure-html/unnamed-chunk-20-1.png)

Since, two types of coordinates are represented, changes to the sample
aesthetics are crucial. The addition of plotting labels also enhance the
interpretability of the plot.

[`biplot`](../reference/biplot.md)`(`[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``HairEyeColor``)``, center ``=`` ``FALSE``)`` ``|>`` `` `` `[`MCA`](../reference/MCA.md)`(``)`` ``|>`` `` `` `[`samples`](../reference/samples.md)`(``col ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"forestgreen"``, ``"orange3"``)``, pch ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``15``, ``17``)``, label ``=`` ``TRUE``, `` `` label.col ``=`` ``"black"``, label.offset ``=`` ``1``)`` ``|>`` `` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](CA_MCA_files/figure-html/unnamed-chunk-21-1.png)

Default arguments for `MCA` will suffice, the only argument that is
worth noting controls the labels of the CLP coordinates in the display.
In the first example `lvl.num = TRUE` was shown, which is the default
value. Detail regarding the levels can also be displayed by changing
this argument to `FALSE`:

[`biplot`](../reference/biplot.md)`(`[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``HairEyeColor``)``, center ``=`` ``FALSE``)`` ``|>`` `` `` `[`MCA`](../reference/MCA.md)`(``lvl.num ``=`` ``FALSE``)`` ``|>`` `` `` `[`samples`](../reference/samples.md)`(``col ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"forestgreen"``, ``"orange3"``)``, pch ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``15``, ``17``)``, label ``=`` ``TRUE``, `` `` label.col ``=`` ``"black"``, label.offset ``=`` ``1``)`` ``|>`` `` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](CA_MCA_files/figure-html/unnamed-chunk-22-1.png)

### Measure of fit

The `fit.mesaures()` function should be utilised to obtain the specific
fit measures as is done in other biplot optpions available in
`biplotEZ`:

`mca.out`` ``<-`` `[`biplot`](../reference/biplot.md)`(`[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``HairEyeColor``)``, center ``=`` ``FALSE``)`` ``|>`` `` `` `[`MCA`](../reference/MCA.md)`(``)`` ``|>`` `[`fit.measures`](../reference/fit.measures.md)`(``)`` `` ``mca.out`` ``#> Object of class biplot, based on 32 samples and 4 variables.`` ``#> 1 numeric variable.`` ``#> 3 categorical variables.`` ``#> `` ``#> Quality of fit = 28.6%`
