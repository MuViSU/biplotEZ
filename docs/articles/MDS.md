...
`sqrtManhattan`` ``#> function (X) `` ``#> {`` ``#> sqrt(stats::dist(X, method = "manhattan"))`` ``#> }`` ``#> <bytecode: 0x803da8548>`` ``#> <environment: namespace:biplotEZ>`` `[`biplot`](../reference/biplot.md)`(``rock``, scaled ``=`` ``TRUE``)`` ``|>`` `[`PCO`](../reference/PCO.md)`(``dist.func ``=`` ``sqrtManhattan``)`` ``|>`` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`
