...
This function allows the user to format the aesthetics for the ordinal
biplot axes.
...
- reverse:
  ...
  logical indicator to switch the narrow to wide calibration on the axis

- lwd.factor:

  a numeric value for increasing line width for each consecutive level
...
- lwd.factor:
  ...
  the factor with which to increase the line width for each level.

- reverse:

  whether the axis goes narrow to wide or vice versa.
